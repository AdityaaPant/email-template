...
export async function POST(request: NextRequest) {
  try {
    ...
    const EmailComponent = emailComponents[templateSlug as keyof typeof emailComponents];
    if (!EmailComponent) {
      return NextResponse.json(
    ...
    const props = sampleProps[templateSlug as keyof typeof sampleProps] || sampleProps.default;

    // Render the email
    const emailHtml = await render(EmailComponent(props));
    const emailTitle = emailTitles[templateSlug as keyof typeof emailTitles] || 'Email Template';

    if (method === 'resend') {
      // Use Resend API (recommended)
      const resendApiKey = process.env.RESEND_API_KEY;
      if (!resendApiKey || resendApiKey === 'your_resend_api_key_here') {
        // Return HTML content for testing without API key
        return NextResponse.json({
          success: true,
          message: 'Email rendered successfully! Configure RESEND_API_KEY to send real emails.',
          html: emailHtml,
          instructions: 'Copy the HTML content below and save as .html file to test locally',
      ...
      console.log('Subject:', `[TEST] ${emailTitle}`);
    ...
    // Fallback method - return the HTML for manual testing
    return NextResponse.json({
      success: true,
